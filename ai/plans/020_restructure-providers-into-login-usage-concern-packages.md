...
5. Update `registry.py` imports to the new `provider.py` modules, and update import lines directly in `cli.py`, `collector.py`, `provider_discovery.py`, and the affected test files.
6. Run `uv run pytest` — full suite must pass unchanged in behavior.

Note: the `ai-usage provider add` wizard flow itself (interactive multi-step credential-kind matching, prompting) is a separate follow-up feature, not part of this pure-structure refactor — this refactor just lays the groundwork (`LoginMethod`/`UsageMethod` split with `credential_kind` metadata) that the wizard would consume later.
