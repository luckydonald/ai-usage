"""Copilot billing usage method: GitHub AI-credit billing HTTP API."""
...
class BillingApiUsage(UsageMethod):
    """GitHub AI-credit billing API (requires an externally-supplied token, no login
    method of its own — matches the original `CopilotBillingProvider`, which had no
    `authenticate`/`discover` and expected the credential to be supplied by CLI flag or
    manual entry)."""
